...
namespace Deadbeef
{
	...
	WindmillLeftFoot::WindmillLeftFoot(b2World& world, Vect2 position, Puzzle* parent):
	PuzzlePiece("Windmill:leftfoot", world, position, parent)
	{
		m_soundEffect = "SFX06";
		...
		b2Vec2 verts[5];
		verts[4] = b2Vec2(-1.2, -1.75);
		verts[3] = b2Vec2(-1.75, -1.0);
		verts[2] = b2Vec2(-1.75, 1.75);
		verts[1] = b2Vec2( 1.75, 1.75);
		verts[0] = b2Vec2( 1.75, -1.75);
		...
		bottomSensorDef.filter.categoryBits = 0x0002;
		bottomSensorDef.filter.maskBits = 0x0002;
		...
		topSensorDef.filter.categoryBits = 0x0008;
		topSensorDef.filter.maskBits = 0x0008;
	...
	WindmillLeftFoot::~WindmillLeftFoot()
	{
		
	}
	
	void WindmillLeftFoot::Update(float elapsedTime)
	{
			...
			while(contactEdges) 
			{
				   ...
				   contactEdges->contact->GetFixtureA()->IsSensor())
				{
					if (((PuzzlePiece*)contactEdges->contact->GetFixtureA()->GetUserData())->IsLinked() )
					{
						...
						((WindmillPuzzle*)mp_parentPuzzle)->SetLeftFootLocked();
						m_linked = true;
						
						SetImage("Windmill:leftfooton");
						frameRate = 3;
