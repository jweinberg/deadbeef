...
namespace Deadbeef
{
	WindmillBody::WindmillBody(b2World& world, Vect2 position, Puzzle* parent):
	PuzzlePiece("Windmill:body", world, position, parent)
	{
		m_soundEffect = "SFX06";
		...
		b2Vec2 verts[8];
		verts[7] = b2Vec2(-2.5, -3.2);
		verts[6] = b2Vec2(-3.4, -2.3);
		verts[5] = b2Vec2(-3.4, 2.8);
		verts[4] = b2Vec2(-3.1, 3.2);
		verts[3] = b2Vec2( 3.1, 3.2);
		verts[2] = b2Vec2( 3.4, 2.8);
		verts[1] = b2Vec2( 3.4, -2.3);
		verts[0] = b2Vec2( 2.5, -3.2);
		...
		bottomSensorShape.m_p = b2Vec2(0, 3.25);
		bottomSensorShape.m_radius = 0.2;
		
		b2CircleShape topSensorShape;
		topSensorShape.m_p = b2Vec2(0, -3.25);
		...
		bottomSensorDef.filter.categoryBits = 0x0020;
		bottomSensorDef.filter.maskBits = 0x0020;
		...
		topSensorDef.filter.categoryBits = 0x0040;
		topSensorDef.filter.maskBits = 0x0040;
	...
	WindmillBody::~WindmillBody()
	{
		
	}
	
	void WindmillBody::Update(float elapsedTime)
	{
